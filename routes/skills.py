...
def _extract_purpose(skill_md_path:Path):
	...
	try:
		A=F.read_text(encoding=_F);B=re.match(_T,A,flags=re.DOTALL)
		if B:
			I=B.group(1);G=re.search('^\\s*description\\s*:\\s*(.+?)\\s*$',I,flags=re.MULTILINE)
			if G:
				C=G.group(1).strip().strip(_c)
				if C:return C[:120]+(H if len(C)>120 else'')
			A=A[B.end():]
		J=[A.strip()for A in A.split(_L)if A.strip()]
		for D in J:
			if D in{'---',_U}:continue
...
def _extract_homepage(skill_md_path:Path):
	...
	try:
		E=A.read_text(encoding=_F);B=re.match(_T,E,flags=re.DOTALL)
		if not B:return''
		F=B.group(1);C=re.search('^\\s*homepage\\s*:\\s*(.+?)\\s*$',F,flags=re.MULTILINE)
		if C:
			D=C.group(1).strip().strip(_c)
			if D.startswith((_M,_V)):return D
		return''
	except Exception:return''
def _extract_required_env(skill_md_path:Path):
	...
	if not A.is_file():return[]
	try:
		D=A.read_text(encoding=_F);B=re.match(_T,D,flags=re.DOTALL)
		if not B:return[]
		E=B.group(1);C=re.search('^\\s*metadata\\s*:\\s*(.+?)\\s*$',E,flags=re.MULTILINE)
		if not C:return[]
		F=json.loads(C.group(1));G=(F.get(_k)or{}).get(_d)or{};H=G.get(_e)or[];return[A for A in H if isinstance(A,str)and A.strip()]
	except Exception:return[]
def _extract_keywords(skill_md_path:Path):
	...
	try:
		E=B.read_text(encoding=_F);C=re.match(_T,E,flags=re.DOTALL)
		if not C:return[]
		F=C.group(1);D=re.search('^\\s*keywords\\s*:\\s*(.+?)\\s*$',F,flags=re.MULTILINE)
		if not D:return[]
		A=D.group(1).strip().strip(_c)
		if not A:return[]
		if A.startswith('['):
...
def _keywords_from_frontmatter(entry:dict):
	'从 frontmatter 条目提取 keywords 列表。';A=entry.get(_l)
...
def _required_env_from_frontmatter(entry:dict):
	...
	if isinstance(B,dict):
		...
		if isinstance(E,dict):
			F=E.get(_d)or{};A=F.get(_e)if isinstance(F,dict)else _C
...
def get_skill_tree(q:str=Query('',description='检索关键词，按名称/描述/keywords 过滤本地技能')):
	...
	if not SKILLS_ROOT.is_dir():return{_S:[]}
	D=get_skill_frontmatter_cache();M=get_skill_ratings_list();E=_ratings_list_to_dict(M);F=(q or'').strip();G=[]
	for A in sorted(D.keys()):
		B=D[A];H=(B.get('description')or'').strip();I=_keywords_from_frontmatter(B)
		if F and not _skill_matches_query(A,H,I,F):continue
		N=A;C=E.get(N,{})or E.get(f"local/{A}",{})
		if not isinstance(C,dict):C={}
		O=(B.get(L)or'').strip();J=_required_env_from_frontmatter(B);K=resolve_skill_dir(SKILLS_ROOT,A);P=[A for A in J if not os.environ.get(A,'').strip()and not(K and skill_env_has_key(K,A))];G.append({'name':A,'purpose':H,_l:I,'rating':C,L:O,'required_env':J,'missing_env':P})
	return{_S:G}
...
def get_env_variable(key:str|_C=Query(_C,description='环境变量名'),skill_name:str|_C=Query(_C,description='技能目录名；若提供则从 skills/<name>/.env 读取，不写全局')):
	'获取环境变量：未传 skill_name 时读进程环境；传 skill_name 时只读该技能目录下 `.env`。';A=key;A=(A or'').strip()
	...
	if C:
		...
		return{_N:A,_g:B,_K:C,_W:_X}
	B=os.environ.get(A,'');return{_N:A,_g:B,_W:_m}
@router.post(_n)
async def set_env_variable(request:Request):
	'\n    设置环境变量。\n    - 请求体含 `skill_name` 时：只写入 `skills/<skill_name>/.env`，不修改全局 `.env` 与进程环境。\n    - 否则：写入运行目录 `.env` 并 `os.environ[key]=value`（与 main 一致）。\n    ';H=request
	try:D=await H.json()
	except Exception:D={}
	A=str(D.get(_N)or'').strip();E=str(D.get(_g)or'').strip();F=str(D.get(_K)or'').strip()
	if not A or not is_valid_env_key(A):raise HTTPException(status_code=400,detail='环境变量名不合法（需大写字母开头，仅含大写字母/数字/下划线）')
	if F:
		I=resolve_skill_dir(SKILLS_ROOT,F)
		if not I:raise HTTPException(status_code=404,detail=f"技能不存在或无效: {F}")
		try:write_skill_env_key(I,A,E)
		except OSError as B:raise HTTPException(status_code=500,detail=f"无法写入技能 .env: {B}")from B
		return{_I:_A,_N:A,_K:F,_W:_X}
	M=getattr(H.app.state,'base_dir',_C)or BASE_DIR;C=M/'.env'
	try:C.parent.mkdir(parents=_A,exist_ok=_A)
	except OSError as B:raise HTTPException(status_code=500,detail=f"无法创建目录 {C.parent}: {B}")from B
	G=[];J=_J
	if C.is_file():
		for K in C.read_text(encoding=_F).splitlines():
			L=K.strip()
			if L.startswith(A+'=')or L.startswith(A+' ='):G.append(f"{A}={E}");J=_A
			else:G.append(K)
	if not J:G.append(f"{A}={E}")
	try:C.write_text(_L.join(G)+_L,encoding=_F)
	except OSError as B:raise HTTPException(status_code=500,detail=f"无法写入 .env 到 {C}: {B}")from B
	os.environ[A]=E;return{_I:_A,_N:A,_W:_m}
...
def get_online_skills(source:str=Query('newest',description='列表来源：newest 最新 / top-downloads 热门下载 / search 搜索'),limit:int=Query(30,ge=1,le=100,description='返回条数'),q:str=Query('',description='搜索关键词（source=search 时使用）')):
	...
	try:
		if E=='search':B=f"{CLAWHUB_API_BASE}/search";C={'q':q or _X,D:A}
...
def _clawhub_proxies():
	...
	if not A:return{}
	if not A.startswith((_M,_V)):A=_M+A
...
def _resolve_skill_download_url(slug:str,download_url:str|_C,version:str|_C,clawhub_url:str|_C):
	...
	if B and B.strip().startswith((_M,_V)):return B.strip()
	A=(A or'').strip()
...
def _extract_archive_to_skill_dir(archive_bytes:bytes,dest_dir:Path,slug:str):
	'\n    将 zip 或 tar.gz 解压到 dest_dir（即 skills/<slug>/）。\n    若压缩包仅含一个顶层目录，则使用该目录内容；否则使用根目录内容。\n    ';I=archive_bytes;F=dest_dir;F.mkdir(parents=_A,exist_ok=_A);M=I[:4]==_o or I[:2]==b'PK'
	if M:
		...
		try:
			...
			if len(E)==1 and next(iter(E))and not next(iter(E)).startswith(_E):
				...
				for C in K:
					if C.startswith(_Y)or _Z in C or'\\.'in C:continue
	...
	with tarfile.open(fileobj=N,mode='r:*')as H:
		...
		if len(E)==1 and next(iter(E))and not next(iter(E)).startswith(_E):
			...
			for D in H.getmembers():
				if _Y in D.name or _Z in D.name:continue
...
def _download_from_url(url:str):
	...
	if not A.startswith((_M,_V)):raise HTTPException(status_code=400,detail='下载链接需以 http:// 或 https:// 开头')
...
def _parse_slug_from_url(url:str):
	...
	for D in(_h,_X,'name'):
		if D in C and C[D]and C[D][0].strip():
			J=C[D][0].strip();A=re.sub(_i,'_',J).strip('_')
	...
	if not B or B.lower()in _URL_SLUG_BLACKLIST:return _O
	A=re.sub(_i,'_',B).strip('_');return A if A and len(A)<=120 else _O
def _validate_skill_archive_and_slug(archive_bytes:bytes,url:str):
	...
	J=D[:4]==_o or D[:2]==b'PK';E=_J;C=_C
	if J:
		with zipfile.ZipFile(io.BytesIO(D),'r')as K:
			F=[A.replace(_D,_B)for A in K.namelist()if A.strip()];A={A.split(_B)[0].strip()for A in F if not A.startswith(_Y)and _Z not in A};A.discard('')
			for G in F:
		...
		try:
			...
			with tarfile.open(fileobj=L,mode='r:*')as M:
				H=[A.replace(_D,_B)for A in M.getnames()if A.strip()and _Y not in A and _Z not in A];A={A.split(_B)[0].strip()for A in H};A.discard('')
				for I in H:
...
def import_skill_from_url(body:ImportFromUrlBody):
	...
	refresh_skill_frontmatter_cache();return{_I:_A,_P:f"已导入技能: {A}",_h:A}
...
def install_skill_from_clawhub(body:SkillInstallBody):
	...
	refresh_skill_frontmatter_cache();return{_I:_A,_P:f"已安装技能: {A}",_h:A}
...
def get_skill_files(skill_name:str=Query(...,description=_a)):
	...
	for(K,D,L)in os.walk(B):
		D[:]=[A for A in D if not A.startswith(_E)and A!=_j];M=Path(K)
		try:G=M.relative_to(B)
		except ValueError:continue
		H=str(G)+_B if str(G)!=_E else''
		for E in sorted(D):
			if E.startswith(_E)or E==_j:continue
			C.append({_b:H+E,_Q:'dir'})
		for I in sorted(L):
			if I.startswith(_E):continue
			F=H+I;J={_b:F,_Q:'file'}
...
async def get_skill_file_content(request:Request,skill_name:str,file_path:str):
	...
	try:
		with tempfile.NamedTemporaryFile(mode='w',suffix='.json',delete=_J,encoding=_F)as L:json.dump(P,L,ensure_ascii=_J);E=L.name
...
def get_skill_file(skill_name:str=Query(...,description=_a),path:str=Query(...,description='相对技能目录的文件路径，如 SKILL.md、scripts/main.py')):
...
def get_skill_routes():
	...
	for A in sorted(os.listdir(SKILLS_ROOT)):
		...
		for(I,E,J)in os.walk(B):
			E[:]=[A for A in E if not A.startswith(_E)and A!=_j];K=Path(I)
			...
			for G in sorted(J):
				...
				D.append({_K:A,_b:C,'route':_build_skill_file_route(A,C)})
	return{H:D}
@router.get('/api/skills/detail')
def get_skill_detail(skill_name:str=Query(...,description=_a)):'获取技能详情：SKILL.md 全文。';return get_skill_file(skill_name=skill_name,path=_G)
@router.put(_f)
def update_skill(body:SkillUpdateBody):
	...
	refresh_skill_frontmatter_cache();return{_I:_A,_P:f"已更新: {A}/{D}"}
@router.delete(_f)
def delete_skill_api(skill_name:str=Query(...,description=_a)):
	...
	refresh_skill_frontmatter_cache();return{_I:_A,_P:f"已删除技能: {A}"}
def _strip_code_fence(text:str):
	'去掉首尾 markdown 代码块包裹。';B=(text or'').strip()
	if not B.startswith(_U):return B
	A=B.split(_L)
	if A[0].startswith(_U):A=A[1:]
	if A and A[-1].strip()==_U:A=A[:-1]
	return _L.join(A)
...
用户修改指令：{instruction}""";C=[]
	try:
		N=J.chat.completions.create(model=K,messages=[{F:'system',_R:L},{F:'user',_R:M}],temperature=0,stream=_A)
		for D in N:
			E=getattr(D.choices[0],'delta',_C)if D.choices else _C;B=getattr(E,_R,_C)if E else _C
			if B:C.append(B);A=json.dumps({_Q:G,G:B},ensure_ascii=_J);yield f"data: {A}\n\n"
		O=_strip_code_fence(''.join(C));A=json.dumps({_Q:'done',_R:O},ensure_ascii=_J);yield f"data: {A}\n\n"
	except Exception as P:A=json.dumps({_Q:'error',_P:str(P)},ensure_ascii=_J);yield f"data: {A}\n\n"
