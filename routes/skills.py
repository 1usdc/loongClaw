...
def _extract_required_env(skill_md_path:Path):
	...
	try:
		...
		if not C:return[]
		F=json.loads(C.group(1));G=(F.get(_f)or{}).get(_g)or{};H=G.get('env')or[];return[A for A in H if isinstance(A,str)and A.strip()]
	except Exception:return[]
...
def _keywords_from_frontmatter(entry:dict):
	'从 frontmatter 条目提取 keywords 列表。';A=entry.get(_h)
...
def _required_env_from_frontmatter(entry:dict):
	'从 frontmatter 条目的 metadata（含 clawdbot.requires.env）提取所需环境变量。';A=entry.get('metadata')or{}
	if isinstance(A,dict):
		B=A.get(_f)or{};C=(B if isinstance(B,dict)else{}).get(_g)or{};D=C.get('env')if isinstance(C,dict)else _C
		if isinstance(D,list):return[A for A in D if isinstance(A,str)and A.strip()]
	return[]
@router.get(_Z)
def get_skill_tree(q:str=Query('',description='检索关键词，按名称/描述/keywords 过滤本地技能')):
	...
	for A in sorted(D.keys()):
		...
		N=(B.get(K)or'').strip();J=_required_env_from_frontmatter(B);O=[A for A in J if not os.environ.get(A,'').strip()];G.append({'name':A,'purpose':H,_h:I,'rating':C,K:N,'required_env':J,'missing_env':O})
	return{_Q:G}
...
def get_env_variable(key:str|_C=Query(_C,description='环境变量名')):
	...
	if not A or not re.match(_i,A):raise HTTPException(status_code=400,detail='环境变量名不合法（需提供 key 查询参数，且为大写字母开头、仅含大写字母/数字/下划线）')
	B=os.environ.get(A,'');return{_a:A,'value':B}
@router.post(_j)
async def set_env_variable(request:Request):
	...
	A=str(E.get(_a)or'').strip();F=str(E.get('value')or'').strip()
	if not A or not re.match(_i,A):raise HTTPException(status_code=400,detail='环境变量名不合法（需大写字母开头，仅含大写字母/数字/下划线）')
	...
	os.environ[A]=F;return{_J:_A,_a:A}
...
def _extract_archive_to_skill_dir(archive_bytes:bytes,dest_dir:Path,slug:str):
	'\n    将 zip 或 tar.gz 解压到 dest_dir（即 skills/<slug>/）。\n    若压缩包仅含一个顶层目录，则使用该目录内容；否则使用根目录内容。\n    ';I=archive_bytes;F=dest_dir;F.mkdir(parents=_A,exist_ok=_A);M=I[:4]==_k or I[:2]==b'PK'
	if M:
...
def _parse_slug_from_url(url:str):
	...
	for D in(_b,'skill','name'):
		if D in C and C[D]and C[D][0].strip():
			J=C[D][0].strip();A=re.sub(_c,'_',J).strip('_')
	...
	A=re.sub(_c,'_',B).strip('_');return A if A and len(A)<=120 else _M
def _validate_skill_archive_and_slug(archive_bytes:bytes,url:str):
	...
	J=D[:4]==_k or D[:2]==b'PK';E=_I;C=_C
	if J:
...
def import_skill_from_url(body:ImportFromUrlBody):
	...
	refresh_skill_frontmatter_cache();return{_J:_A,_N:f"已导入技能: {A}",_b:A}
...
def install_skill_from_clawhub(body:SkillInstallBody):
	...
	refresh_skill_frontmatter_cache();return{_J:_A,_N:f"已安装技能: {A}",_b:A}
...
def get_skill_files(skill_name:str=Query(...,description=_W)):
	...
	for(K,D,L)in os.walk(B):
		D[:]=[A for A in D if not A.startswith(_E)and A!=_d];M=Path(K)
		try:G=M.relative_to(B)
		except ValueError:continue
		H=str(G)+_B if str(G)!=_E else''
		for E in sorted(D):
			if E.startswith(_E)or E==_d:continue
...
def get_skill_routes():
	...
	for A in sorted(os.listdir(SKILLS_ROOT)):
		...
		for(I,E,J)in os.walk(B):
			E[:]=[A for A in E if not A.startswith(_E)and A!=_d];K=Path(I)
			...
			for G in sorted(J):
				...
				D.append({_e:A,_X:C,'route':_build_skill_file_route(A,C)})
	return{H:D}
