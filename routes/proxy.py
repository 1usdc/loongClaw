...
async def proxy_url(request:Request,url:str|None=Query(None,description='要由后端代为请求的 http(s) 地址')):
	...
	try:
		...
		if C.method==_H:J=await C.form();K={A:B for(A,B)in J.items()};B=requests.post(A,data=K,timeout=PROXY_TIMEOUT,headers=F,allow_redirects=_I)
		else:
			G={A:B for(A,B)in C.query_params.items()if A!=_K};H=A
			if G:L='&'if'?'in A else'?';H=A+L+urlencode(G)
		...
		if _F in D:M=_E;E=_rewrite_html_links(E,B.url,M)
		N={_N:_O}if _F in D else{};return Response(content=E,media_type=D or _F,headers=N)
	except requests.RequestException as I:logger.warning('代理请求失败 url=%s: %s',A,I);return Response(content=f"代理请求失败: {I!s}",status_code=502,media_type=_B)
...
async def proxy_sub_request(request:Request,sub_path:str):
	...
	try:
		J=urlparse(G);N=parse_qs(J.query)if J.query else{};F=N.get(_K)
	...
	O=urlparse(B)
	if not O.netloc:return Response(content='Invalid base URL',status_code=400,media_type=_B)
	...
	try:
		...
		if C.method==_H:P=await C.form();Q={A:B for(A,B)in P.items()};D=requests.post(A,data=Q,timeout=PROXY_TIMEOUT,headers=L,allow_redirects=_I)
		...
		if _F in H:R=_E;I=_rewrite_html_links(I,D.url,R)
		S={_N:_O}if _F in H else{};return Response(content=I,media_type=H or'application/octet-stream',headers=S)
	except requests.RequestException as M:logger.warning('子资源代理失败 target=%s: %s',A,M);return Response(content=f"代理请求失败: {M!s}",status_code=502,media_type=_B)